...
module.exports = {
	...
	addSms: function(numtel, smsvalue){
		var idEleveSms;
		var queryString = "SELECT ID_ELEVE FROM ELEVE WHERE TELEPHONE_ELEVE=?";

		db.query(queryString, [numtel], function(err,ideleve){
			if(err) throw err;
			ideleve = ideleve[0].ID_ELEVE
			if (ideleve == null){ // Si l'eleve n'existe pas
				console.log(' connu')
 				var queryString = "INSERT INTO ELEVE (`TELEPHONE_ELEVE`) VALUES (?);";
				db.query(queryString, [numtel], function(err,idelevecreated, callback){
					if(err) throw err;
					idEleveSms = idelevecreated;
				});
				
			} else { // Si l'eleve est connu
				console.log('Eleve inconnu')
				idEleveSms = ideleve ;
			}

			var queryString = "INSERT INTO SMS_RECU(`ID_ELEVE`, `TELEPHONE_SMS`, `VALEUR_SMS`) VALUES (?,?,?);";
			console.log(queryString);
			db.query(queryString, [idEleveSms, numtel, smsvalue], function(err,rows){
				if(err) throw err;
				console.log(rows);
			});
