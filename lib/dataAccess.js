...
module.exports = {
	...
	editGoodAnswer: function(id_good_answer, ID_QUESTION, callback){
		var queryString = "UPDATE QUESTION SET ID_REPONSE_BONNE=? WHERE ID_QUESTION=?";
	...
	reqExportInfo: function(id_questionnaire, callback){
		var queryString = 'SELECT * FROM QUESTIONNAIRE q JOIN QUESTION qu ON qu.ID_QUESTIONNAIRE=q.ID_QUESTIONNAIRE join REPONSE r ON r.ID_QUESTION=qu.ID_QUESTION WHERE q.ID_QUESTIONNAIRE=?';
		db.query(queryString, [id_questionnaire], function(err,rows){
			if(err) throw err;
			callback(rows);
		});
