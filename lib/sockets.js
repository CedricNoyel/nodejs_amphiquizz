...
module.exports = function(http){
	...
	io.on('connection', function(socket) {
	    ...
	    socket.on('req_launch_questionnaire', function(id_session, anonyme){
	    	dataAccess.reqCreateSession(sess.id_professeur, id_session, function(idsession){
				global.session = idsession.insertId;
				if (global.FAKE_SMS){
	    		...
	    		console.log("Lancement d'une session sur la session " + id_session + " avec anonyme: " + anonyme);
	    	});
	    });
	    ...
	    socket.on('req_stop_questionnaire', function(id_session){
	    	dataAccess.reqStopSession(id_session, function(result){
	    		...
	    		console.log('Questionnaire quitté sur la session ' + id_session);
	    	});
	    });
	    ...
	    socket.on('req_stats_question', function(idSess, idQuestion){
	    	dataAccess.findSMS(idSess, idQuestion, function(data){
	    		socket.emit('res_stats_question', data);
	    	});
	    });

	    socket.on('req_id_questionnaire_by_id_question', function(idquestion){
	    	dataAccess.reqIdQuestionnaire(idquestion, function(idquestionnaire){
	    		socket.emit('res_id_questionnaire_by_id_question', idquestionnaire);
	    	});
	    });

	    // HISTORIQUE SESSION
	    socket.on('req_historique_sessions', function(idQuestionnaire){
	    	dataAccess.findByFieldHasValue('SESSION', 'ID_QUESTIONNAIRE', idQuestionnaire, function(res){
	    		socket.emit('res_historique_sessions', res);
	    	});
	    });
		// DELETING SESSION
