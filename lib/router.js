...
module.exports = function(app){
	...
	app.get('/admin/questionnaire/:id', [functions.requireLogin], function(req, res){
	    functions.checkIdQuestionnaire(req.session, req.params.id, function(ok){
			if(!ok) {
				res.redirect('/rip');
			} else {
				res.writeHead(200, {"Content-Type": "text/html"});
	...
	app.get('/admin/question/:id', [functions.requireLogin], function(req, res){
		functions.checkIdQuestion(req.session, req.params.id, function(ok){
			if (!ok) { 
				res.redirect('/rip');
			} else {
				res.writeHead(200, {"Content-Type": "text/html"});
